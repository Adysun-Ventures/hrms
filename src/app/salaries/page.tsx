...
export default function SalariesPage() {
  ...
  const autoGenerateMissing = searchParams?.get('autoGenerateMissing') === '1';
  const employeeIdsParam = searchParams?.get('employeeIds') || '';
  const autoGenerateFyStartParam = Number(searchParams?.get('fyStart') || 0) || 0;
  const autoGenerateAiMode = searchParams?.get('aiMode') === '1';
  const [isBatchGeneratingMissing, setIsBatchGeneratingMissing] = useState(false);
...
const handleDownload = async (salary: Salary) => {
  ...
  const createMissingSalariesForEmployee = async (targetEmployeeId: string, fyStart?: number) => {
    const employments = await getEmploymentsByEmployee(targetEmployeeId);
    if (!employments?.length) return { created: 0, failed: 0 };

    const salarySnap = await getDocs(
      query(collection(db, 'salaries'), where('employeeId', '==', targetEmployeeId))
    );
      ...
      salarySnap.docs
        ...
        .map((s: any) => {
          ...
          if (fyStart) {
            ...
            const fyEndDate = new Date(fyStart + 1, 2, 31);
            if (date < fyStartDate || date > fyEndDate) return null;
          }
          return `${year}-${month}`;
        })
    ...
    const chronologicalEmployments = [...(employments as any[])].sort((a: any, b: any) => {
    ...
    let createdCount = 0;
    let failedCount = 0;
    const fyStartDate = fyStart ? new Date(fyStart, 3, 1) : null;
    const fyEndDate = fyStart ? new Date(fyStart + 1, 2, 31) : null;

    for (const employment of chronologicalEmployments) {
      const periodStart = getPeriodStart(employment);
      if (!periodStart) continue;
      const periodEnd = getPeriodEnd(employment);
      if (periodStart > periodEnd) continue;

      const effectiveStart = fyStartDate && periodStart < fyStartDate ? fyStartDate : periodStart;
      const effectiveEnd = fyEndDate && periodEnd > fyEndDate ? fyEndDate : periodEnd;
      if (effectiveStart > effectiveEnd) continue;

      const increments: any[] = Array.isArray(employment?.increments) ? employment.increments : [];
      const incrementTimeline = increments
        .filter((inc) => inc?.incrementDate)
        .map((inc) => ({ ...inc, incrementDateObj: new Date(inc.incrementDate) }))
        .filter((inc) => !Number.isNaN(inc.incrementDateObj.getTime()))
        .sort((a, b) => a.incrementDateObj.getTime() - b.incrementDateObj.getTime());

      const baseCtc =
        Number(employment?.joiningCtc ?? employment?.salary ?? employment?.incrementedCtc ?? 0) || 0;
      const baseVariable = Number(employment?.joiningVariablePay ?? employment?.currentVariablePay ?? 0) || 0;
      ...
      const startMonth = monthStart(effectiveStart);
      const endMonth = monthStart(effectiveEnd);

      for (let current = new Date(startMonth); current <= endMonth; current = addMonths(current, 1)) {
        const month = current.getMonth() + 1;
        const year = current.getFullYear();
        const key = `${year}-${month}`;
        if (paidKeySet.has(key)) continue;

        const applicableIncrement = incrementTimeline
          .filter((inc) => inc.incrementDateObj <= monthEnd(current))
          .slice(-1)[0];

        const ctc = Number(applicableIncrement?.incrementedCtc ?? applicableIncrement?.newSalary ?? baseCtc) || 0;
        const variablePay = Number(applicableIncrement?.incrementVariablePay ?? baseVariable) || 0;
        const fixedPay =
          Number(applicableIncrement?.incrementFixedPay ?? (ctc - variablePay)) || Math.max(0, ctc - variablePay);
        const pfIncluded =
        ...
        try {
          ...
          console.error('Failed to auto-create salary:', { targetEmployeeId, month, year, error });
          failedCount += 1;
        }
      }
    }
  ...
  useEffect(() => {
    if (!autoGenerateMissing || hasAutoGeneratedRef.current) return;

    const targetEmployeeIds = employeeIdsParam
      .split(',')
      .map((id) => id.trim())
      .filter(Boolean);
    ...
    (async () => {
      try {
        setIsBatchGeneratingMissing(true);
        toast.loading(
          autoGenerateAiMode
            ? 'Generating missing salaries with AI for selected employees...'
            : 'Generating missing salaries for selected employees...',
          { id: 'batch-missing-salary' }
        );

        let totalCreated = 0;
        let totalFailed = 0;
        for (const targetEmployeeId of targetEmployeeIds) {
          const result = await createMissingSalariesForEmployee(
            targetEmployeeId,
            autoGenerateFyStartParam || undefined
          );
        ...
        if (totalCreated > 0) {
          toast.success(
            `${
              autoGenerateAiMode ? 'AI created' : 'Created'
            } ${totalCreated} salaries for ${targetEmployeeIds.length} employees${totalFailed ? `, failed ${totalFailed}` : ''}.`,
            { id: 'batch-missing-salary' }
          );
        } else {
          toast.error(
            totalFailed > 0 ? `No salary created. Failed ${totalFailed}.` : 'No missing salaries found to generate.',
            { id: 'batch-missing-salary' }
          );
        }
      } catch (error: any) {
        console.error('Batch missing salary generation failed:', error);
        toast.error(error?.message || 'Failed to generate missing salaries.', {
          id: 'batch-missing-salary',
        });
      } finally {
        setIsBatchGeneratingMissing(false);
      }
    })();
  }, [autoGenerateMissing, employeeIdsParam, autoGenerateFyStartParam, autoGenerateAiMode, queryClient]);
